...
import com.example.myapplication.model.County;
...
public class ChooseAreaActivity extends Activity {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if(currentLevel==LEVEL_PROVINCE){
                    ...
                    Log.e("MyActivity","item被点击======省级"+provinceList.get(position).getProvinceCode()+" id:"+provinceList.get(position).getId());
                    queryCities();
                }else if (currentLevel==LEVEL_CITY){
                    selectedCity=cityList.get(position);
                    Log.e("MyActivity","item被点击======市级:code"+selectedCity.getCityCode()+"  id："+selectedCity.getId());
                    queryCounties();
    ...
    private void queryCounties() {
        ...
        Log.d("MyActivity","是否有数据："+countyList.size());
        if(countyList.size()>0){
            dataList.clear();
            Log.d("MyActivity","数据为："+countyList.size());
